...
function one_signal_push_notification( $title = '', $message = '', $user_ids = [] ) {
	...
	$body_json_encoded = json_encode( $fields );
...
function sendNotificationToUser( $userId, $orderId, $previous_status, $next_status ) {
	...
	one_signal_push_notification( $title, $message, [ $userId ] );
}
...
function sendNewOrderNotificationToVendor( $order_seller_id, $order_id ) {
	...
	one_signal_push_notification( $title, $message, [ $order_seller_id ] );
}

function wcfm_message_on_new_order( $order_id ) {
	global $WCFM, $wpdb;
	if ( get_post_meta( $order_id, '_wcfm_new_order_notified', true ) ) {
		return;
	}
	$author_id        = -2;
	$author_is_admin  = 1;
		...
		if ( $product_id ) {
			...
			if ( $message_to ) {
				if ( apply_filters( 'wcfm_is_allow_itemwise_notification', true ) ) {
					$wcfm_messages = sprintf( __( 'You have received an Order <b>#%1$s</b> for <b>%2$s</b>', 'wc-frontend-manager' ), '<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url( $order_id ) . '">' . $order->get_order_number() . '</a>', get_the_title( $product_id ) );
			...
			if ( is_a( $order, 'WC_Order' ) ) {
				...
				if ( ! empty( $items ) ) {
					foreach ( $items as $order_item_id => $item ) {
						...
						if ( in_array( $vendor_id, $processed_vendors ) ) {
							continue;
						}
...
function customProductResponse( $response, $object, $request ) {
	...
	foreach ( $attributes as $key => $attr ) {
		...
		$attr['options']  = empty( $attrOptions ) ? array_map(
			function ( $v ) {
				return [
					'name' => $v,
					'slug' => $v,
...
function validateCookieLogin( $cookie ) {
	...
	if ( isset( $cookie ) && strlen( $cookie ) > 0 ) {
		...
		if ( $userId == false ) {
			return new WP_Error( 'expired_cookie', 'Your session has expired. Please logout and login again.', [ 'status' => 401 ] );
		} else {
			return $userId;
		}
	} else {
		return new WP_Error( 'invalid_login', 'Cookie is required', [ 'status' => 401 ] );
	}
}

function checkWhiteListAccounts( $user_id ) {
	$whiteList = [ 'vendor@demo.com', 'delivery_demo', 'demo' ];
