...
defined( 'ABSPATH' ) or wp_die( 'Can\'t access directly.' );
...
class MstoreCheckOut {
	...
	public function __construct() {
		...
		*/
		...
		add_action( 'wp_print_scripts', [ $this, 'handle_received_order_page' ] );
		...
		register_activation_hook( __FILE__, [ $this, 'create_custom_mstore_table' ] );

		/**
		 * Register js file to theme
		 */
		function maanstore_scripts() {
			wp_enqueue_script( 'my_script', plugins_url( 'assets/js/maanstore-api.js', __FILE__ ), [ 'jquery' ], '1.0.0', true );
			wp_localize_script( 'my_script', 'MyAjax', [ 'ajaxurl' => admin_url( 'admin-ajax.php' ) ] );
		}

		add_action( 'wp_enqueue_scripts', 'maanstore_scripts' );
		// Setup Ajax action hook
		add_action( 'wp_ajax_mstore_delete_json_file', [ $this, 'mstore_delete_json_file' ] );
		add_action( 'wp_ajax_mstore_update_limit_product', [ $this, 'mstore_update_limit_product' ] );
		add_action( 'wp_ajax_mstore_update_firebase_server_key', [ $this, 'mstore_update_firebase_server_key' ] );
		add_action( 'wp_ajax_mstore_update_new_order_title', [ $this, 'mstore_update_new_order_title' ] );
		add_action( 'wp_ajax_mstore_update_new_order_message', [ $this, 'mstore_update_new_order_message' ] );
		add_action( 'wp_ajax_mstore_update_status_order_title', [ $this, 'mstore_update_status_order_title' ] );
		add_action( 'wp_ajax_mstore_update_status_order_message', [ $this, 'mstore_update_status_order_message' ] );

		// listen changed order status to notify
		add_action( 'woocommerce_order_status_changed', [ $this, 'track_order_status_changed' ], 9, 4 );
		add_action( 'woocommerce_checkout_update_order_meta', [ $this, 'track_new_order' ] );
		add_action( 'woocommerce_rest_insert_shop_order_object', [ $this, 'track_api_new_order' ], 10, 4 );
	...
	public function handle_received_order_page() {
		...
		if ( is_order_received_page() ) {
			$detect = new MDetect();
			if ( $detect->isMobile() ) {
				wp_register_style( 'maanstore-order-custom-style', plugins_url( 'assets/css/maanstore-order-style.css', MSTORE_PLUGIN_FILE ) );
...
function mstore_plugin_setup_menu() {
	add_menu_page( 'MaanStore Api', 'MaanStore Api', 'manage_options', 'maanstore-plugin', 'mstore_init' );
}

function mstore_init() {
	load_template( dirname( __FILE__ ) . '/templates/maanstore-api-admin-page.php' );
}
