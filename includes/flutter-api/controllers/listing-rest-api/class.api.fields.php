...
class FlutterTemplate extends WP_REST_Posts_Controller {
	...
	public function register_add_more_fields_to_rest_api() {
		...
		if ( $this->_isMyListing ) {
			...
			register_rest_field(
				...
				'newaddress',
				[
					'get_callback' => [ $this, '_rest_get_address_data' ],
				]
			);
			...
			register_rest_field(
				...
				'newlat',
				[
					'get_callback' => [ $this, '_rest_get_lat_data' ],
				]
			);
			...
			register_rest_field(
				...
				'newlng',
				[
					'get_callback' => [ $this, '_rest_get_lng_data' ],
				]
			);
		}
	...
	public function check_availability( $request ) {
		...
		if ( isset( $request['adults'] ) ) {
			$multiply = $request['adults'];
		}
		if ( isset( $request['tickets'] ) ) {
			$multiply = $request['tickets'];
		}
	...
	public function update_slots( $request ) {
		...
		if ( is_array( $_slots_for_day ) && ! empty( $_slots_for_day ) ) {
			foreach ( $_slots_for_day as $key => $slot ) {
				...
				$result              = Listeo_Core_Bookings_Calendar::get_slots_bookings(
					$date_start,
					$date_end,
					[
	...
	static function insert_booking( $args ) {
		...
		if ( isset( $args['expiring'] ) ) {
			$insert_data['expiring'] = $args['expiring'];
		}
		if ( isset( $args['status'] ) ) {
			$insert_data['status'] = $args['status'];
		}
		if ( isset( $args['price'] ) ) {
			$insert_data['price'] = $args['price'];
		}
	...
	public function booking( $object ) {
		...
		switch ( $listing_meta['_listing_type'][0] ) {
			case 'event':
				...
				$booking_id = self::insert_booking(
					[
	...
	public function get_comments_ratings( $object ) {
		...
		foreach ( $comments as $comment ) {
			...
			$total          = $total + (float) $current_rating;
		}
	...
	public function submitReview( WP_REST_Request $request ) {
		if ( $this->_isListingPro ) {
			...
			listing_set_metabox( 'rating', (float) $request['rating'], $postID );
...
class TemplateSearch extends FlutterTemplate {
	...
	*/
	public function register_fields_for_search_advance() {
		...
		register_rest_route(
			'search/v1',
			$this->_isListingPro ? 'listing' : 'job_listing',
			[
				'methods'             => 'GET',
				'callback'            => [
					$this,
	...
	public function searchQuery( $request ) {
		...
		foreach ( $data as $listing ) {
			...
			foreach ( $listing['job_listing_category'] as $value ) {
				...
				if ( ! isset( $listings_grouped[ $type ] ) ) {
					$listings_grouped[ $type ] = [];
				}
