...
class FlutterPaidMembershipsPro extends FlutterBaseController {
	 ...
	 * @var string
	 */
	protected $namespace = 'maan-paid-membership-pro';
	...
	public function __construct() {
		add_action( 'rest_api_init', [ $this, 'register_flutter_paid_memberships_pro_routes' ] );
	}
	...
	public function register_flutter_paid_memberships_pro_routes() {
		...
		register_rest_route(
			...
			[
				[
					'methods'             => 'POST',
					'callback'            => [ $this, 'membership_register' ],
					'permission_callback' => function () {
						return parent::checkApiPermission();
					},
				],
		...
		if ( ! empty( $user_id ) && ! is_wp_error( $user_id ) ) {
			...
			if ( pmpro_changeMembershipLevel( $custom_level, $user_id, 'changed' ) ) {
				...
				if ( empty( $current_user->membership_level ) ) {
					...
					$confirmation_message = '<p>' . sprintf( __( 'Thank you for your membership to %1$s. Your %2$s membership is now active.', 'paid-memberships-pro' ), get_bloginfo( 'name' ), $current_user->membership_level->name ) . '</p>';
				}
