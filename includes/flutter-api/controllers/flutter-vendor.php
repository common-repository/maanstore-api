...
class FlutterVendor extends FlutterBaseController {
	 ...
	 * @var string
	 */
	protected $namespace    = 'wc/v2/maan-flutter'; // Prefix must be wc/ or wc- to reuse check permission function in woo commerce.
	protected $namespace_v3 = 'wc/v3/maan-flutter';
	...
	function set_vendor_vacation_option( $request ) {
		...
		$is_seller = in_array( 'seller', $user->roles ) || in_array( 'wcfm_vendor', $user->roles ) || in_array( 'administrator', $user->roles );
		if ( $is_seller ) {
			if ( is_plugin_active( 'wc-multivendor-marketplace/wc-multivendor-marketplace.php' ) ) {
				global $WCFM, $WCFMmp;
				$is_marketplace      = wcfm_is_marketplace();
	 ...
	 * @return obj product data
	 */
	public function flutter_create_product( $request ) {
		$cookie  = sanitize_text_field( $request['cookie'] );
		$user_id = validateCookieLogin( $cookie );
		...
		if ( $request['status'] != null ) {
			$request_status = $request['status']; // Available status: draft,publish,pending
		}
		...
		if ( $is_seller ) {
			$args = array(
				...
				'post_status'  => $request_status, // (Draft | Pending | Publish)
			...
			// Available product type: simple, grouped, product-external, variable.

			$product->save();
			$product = wc_get_product( $post_id );

			if ( isset( $request['categories'] ) && count( $request['categories'] ) > 0 ) {
				$product->set_category_ids( array( $request['categories'][0]['id'] ) );
				$product->save();
			}
	...
	public function prepeare_product_response( $response, $object, $request ) {
		...
		if ( is_plugin_active( 'dokan-lite/dokan.php' ) ) {
			...
			$dataStore     = array_merge( $dataStore, apply_filters( 'dokan_rest_store_additional_fields', array(), $store, $request ) );
