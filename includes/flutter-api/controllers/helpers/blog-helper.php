...
class FlutterBlogHelper {
	...
	public function get_blog_from_dynamic_link( $request ) {
		...
		return new WP_Error( 'invalid_url', 'Not Found', [ 'status' => 404 ] );
	}
	...
	public function create_blog( $request ) {
		...
		if ( isset( $token ) ) {
			...
			return new WP_Error( 'unauthorized', 'You are not allowed to do this', [ 'status' => 401 ] );
		}
		...
		if ( $status == 'publish' || $status == 'published' ) {
			return new WP_Error( 'unauthorized', 'You are not allowed to publish this post', [ 'status' => 401 ] );
		}
		$my_post = [
			'post_author'   => $user_id,
			'post_title'    => $title,
		...
		if ( comments_open( $post_id ) ) {
			$current_user = get_user_by( 'ID', $user_id );
				...
				if ( ! is_wp_error( $comment_id ) ) {
					...
					return new WP_Error( 'error', $comment_id, [ 'status' => 400 ] );
				}
		} else {
			return new WP_Error( 'comments_open', "This post doesn't allow to  comment", [ 'status' => 400 ] );
		}
	}
}
