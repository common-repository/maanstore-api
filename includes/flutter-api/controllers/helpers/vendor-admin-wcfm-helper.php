...
class VendorAdminWCFMHelper {
	...
	function wcfm_get_commission_by_vendor(
		...
		if ( $marketplece == 'wcvendors' ) {
			...
			$order_status     = apply_filters(
				'wcfm_dokan_allowed_order_status',
				[
					'completed',
					'processing',
					'on-hold',
	...
	function wcfm_update_order_delivery_boys_meta( $order_id, $delivery_boys_array = [] ) {
	...
	function wcfm_get_order_delivery_boys( $order_id, $order_item_id = '' ) {
		...
		if ( ! empty( $delivery_boys ) ) {
			foreach ( $delivery_boys as $delivery_boy ) {
				$delivery_boys_array[] = [
					'order'        => $order_id,
					'item'         => $delivery_boy->item_id,
					'vendor'       => $delivery_boy->vendor_id,
					'delivery_boy' => $delivery_boy->delivery_boy,
					'status'       => $delivery_boy->delivery_status,
				];
			}
		}
