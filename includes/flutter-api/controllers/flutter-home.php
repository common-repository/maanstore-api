...
class FlutterHome extends WP_REST_Controller {
	...
	public function __construct() {
		add_action( 'rest_api_init', [ $this, 'register_flutter_routes' ] );
		add_filter( 'wp_rest_cache/allowed_endpoints', [ $this, 'wprc_add_flutter_endpoints' ] );
	}
	...
	public function register_flutter_routes() {
		$cache = [
			...
			[
				...
				'permission_callback' => [ $this, 'flutter_get_items_permissions_check' ],
			],
		];
	...
	public function flutter_get_items_permissions_check() {
		return get_option( 'maanstore_purchase_code' ) === '1';
	}
	...
	private function get_config_file_path( $lang ) {
		if ( ! isset( $lang ) ) {
			...
			if ( ! empty( $configs ) ) {
				...
				return new WP_Error( 'existed_config', "Config file hasn't been uploaded yet.", [ 'status' => 400 ] );
			}
		} else {
	...
	public function get_category_data( $request ) {
		...
		if ( count( $ids ) > 0 ) {
			...
			return new WP_Error( 'empty_ids', 'categoryIds is empty', [ 'status' => 400 ] );
		}
	}
	...
	public function get_widgets_data( $request ) {
		...
		if ( file_exists( $path ) ) {
			...
			if ( isset( $array['Widgets'] ) ) {
				...
				return new WP_Error( 'invalid_format', "The config file doesn't have 'Widgets' property", [ 'status' => 400 ] );
			}
		} else {
			return new WP_Error( 'existed_config', "Config file hasn't been uploaded yet.", [ 'status' => 400 ] );
		}
